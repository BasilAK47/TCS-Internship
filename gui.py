...
import time
title="Fundus Image generation using GAN"
path1="sample.jpg"
path2="sample1.jpg"
main_color='#271745'  
...
def showcheck():
    ...
    global lb1
    b2=Button(f4,text="Generate",font="Verdana 10 bold",command=lambda:process1(path1,lb1))
    b2.pack(pady=2)
...
from test2 import predict1
import pathlib


def process1(path2,lb1):
    global f6,f2,top
    ret=predict1(path2,lb1)
    f6.pack_forget()
    f6=Frame(f2)
...
if __name__=="__main__":
    ...
    try:
        ...
        image = Image.open(path3)
        messagebox.showerror("Not an image","Choose an image") 
    photo = ImageTk.PhotoImage(image.resize((1800, 700), Image.ANTIALIAS))
    label = Label(f, image=photo, bg=main_color)
    label.image = photo
    label.pack()
    l=Label(f,text=title,font = "Verdana 30 bold",fg="white",bg=main_color)
    l.place(x=50,y=50)
